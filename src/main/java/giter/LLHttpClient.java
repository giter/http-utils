...
public abstract class LLHttpClient {
	...
	protected static SimpleEntry<URLConnection, String> content(
	...
	protected static String encoding(URLConnection conn) {
		...
		for (String cs : new String[] { "charset=", "charset =" }) {
			if (contentType.contains(cs)) {
				return contentType
						.substring(contentType.indexOf(cs) + cs.length())
						.trim().toLowerCase();
			}
		}
	...
	protected static InputStream getInputStream(URLConnection conn)
			throws IOException {
	 ...
	 */
	protected static URLConnection method(URLConnection conn, String method)
			throws ProtocolException {
		if (method != null) {
	...
	protected static URLConnection postMultipart(URLConnection conn,
		...
		try (NMultiPartOutputStream mos = new NMultiPartOutputStream(
	...
	protected static URLConnection postQuery(URLConnection conn,
